...
class Videos(HTTPEndpoint):
    ...
    async def post(self, request: Request) -> JSONResponse:
        ...
        async with aiofiles.open(f"{settings.VIDEOS_PATH}/{filename}", 'wb') as new_video:
            written_bytes = await new_video.write(contents)
            if written_bytes != len(contents):
                raise HTTPException(status_code=400, contents="Couldn't save video file to server.")
...
class Video(HTTPEndpoint):
    ...
    async def get(self, request: Request) -> StreamingResponse:
        ...
        return StreamingResponse(video_generator(str(path_to_video)), media_type="video/mp4")